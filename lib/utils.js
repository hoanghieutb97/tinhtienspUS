...
export function tinhTienMuc(lop, giaVatLieu) {
    ...
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            tongtien = tongtien + 800;
            continue;
        }
...
export function tinhTienTK(lop, giaVatLieu) {
    console.log(lop);
    let gia = giaVatLieu.filter(item => item.nameCode == "luongthietke")[0].price;
    let tienCongPhut = gia / (26 * 8 * 60);
    let tongtien = 0
    let sttTheu = false;
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            let tienx = Math.round(lop[i].chieuDai * lop[i].chieuRong * 2.54 * 2.54 * 27000 * lop[i].soMatIn * tienCongPhut / (4 * 700 * 100))
            tongtien = tongtien + (tienx < 15000 ? 15000 : tienx)
            sttTheu = true;
        }
    }
    console.log(tongtien);
    ...
    tongtien = lop.length * tienCongPhut * 1;

    return tongtien
}
export function tinhTienIn(lop, giaVatLieu) {
    let tongtien = 0;
    let gia = giaVatLieu.filter(item => item.nameCode == "luongthoin")[0].price;
    let tienCongPhut = gia / (26 * 8 * 60);

    let sttTheu = false;
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            tongtien = tongtien + 8000 + Math.round(lop[i].chieuDai * lop[i].chieuRong * 2.54 * 2.54 * 27000 * lop[i].soMatIn * tienCongPhut / (3 * 700 * 100))
            sttTheu = true;
        }
    }
    if (sttTheu) {
...
export function tinhTienCat(lop, giaVatLieu) {
    let tongtien = 0;
    let gia = giaVatLieu.filter(item => item.nameCode == "luongthocat")[0].price;
    let tienCongPhut = gia / (26 * 8 * 60);
    let sttTheu = false;
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            if (lop[i].catStatus == true) tongtien = tongtien + 2000
            sttTheu = true;
        }
    }
    if (sttTheu) {
    ...
    for (let i = 0; i < lop.length; i++) {
        ...
        else if (chatLieu.slice(-4) == "15mm") tienin = tienin * 7.5
...
export function tinhTienDIen(lop, giaVatLieu) {
    ...
    let giaIN = giaVatLieu.filter(item => item.nameCode == "luongthoin")[0].price;
    let giaCAT = giaVatLieu.filter(item => item.nameCode == "luongthocat")[0].price;
    let tienDien = giaVatLieu.filter(item => item.nameCode == "tiendien")[0].price;

    let tienCongPhutIN = giaIN / (26 * 8 * 60);
    let tienCongPhutCAT = giaCAT / (26 * 8 * 60);
    let tongtien = 0;
    let sttTheu = false;
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            tongtien = tongtien + 500
            sttTheu = true;
        }
    }
    if (sttTheu) {
    ...
    tongtien = (5 * tienDien * tienIn) / (tienCongPhutIN * 60) + (3 * tienDien * tienCat) / (tienCongPhutCAT * 60);
...
export function tinhTienChietKhau(lop, giaVatLieu) {
    ...
    let haomonmaymoc = giaVatLieu.filter(item => item.nameCode == "haomonmaymoc")[0].price;

    let heSo = Number(haomonmaymoc);
    for (let i = 0; i < lop.length; i++) {
        let downTien = 0;
        let chieuDai = (+ lop[i].chieuDai);
        let chieuRong = (+lop[i].chieuRong);
...
export function tinhTienKeoDan(lop, giaVatLieu, thongSoTong) {
    ...
    for (let i = 0; i < lop.length; i++) {
        if (lop[0].chatLieu.substring(0, 4) === "theu") {
            return 0
        }
        let chieuDai = (+ lop[i].chieuDai);
        let chieuRong = (+lop[i].chieuRong);
...
export function tinhCanNang(lop, giaVatLieu, thongSoTong, giaPhuKien) {
    ...
    return (+thongSoTong.canVl + (+ thongSoTong.canHop));

}
export function tinhCanNangUocLuong(lop, giaVatLieu, thongSoTong, giaPhuKien) {
    ...
    for (let i = 0; i < thongSoTong.phuKien.length; i++) {
        ...
        if (giaPhuKien.filter(item => item._id == tenChatLieu).length != 0) {
...
export function tinhTienDongGoi(lop, giaVatLieu, thongSoTong) {
    ...
    let gia = giaVatLieu.filter(item => item.nameCode == "luongdonggoi")[0].price;
...
export function tinhTienMatBang(lop, giaVatLieu, thongSoTong, giaPhuKien) {
    let gia = giaVatLieu.filter(item => item.nameCode == "chiphimatbang")[0].price;
    return gia
}