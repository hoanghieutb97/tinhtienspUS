...
export default function SelectChatLieuLop(props) {
    ...
    const fuse = new Fuse(vatLieu, options);
    // const searchResult = fuse.search(textSearch);
    const searchResult = textSearch ? fuse.search(textSearch).map(result => result.item) : vatLieu; // Nếu textSearch rỗng, trả về toàn bộ mảng
